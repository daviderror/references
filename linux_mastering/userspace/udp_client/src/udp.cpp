...
namespace udp {
    Udp::Udp (sockaddr_in &udp, udp_init init, int &socket_d) {
        socket_d = socket(init.family, SOCK_DGRAM, 0);
    ...
    int Udp::open(sockaddr_in &udp_sock, int &socket_d) {
        if ((bind(socket_d,(const sockaddr *) &udp_sock, sizeof(sockaddr_in))) < 0) {
